...
fun FunFactScreen(
    ...
    var favouriteBoolean : Boolean by remember { mutableStateOf<Boolean>(favourite) }
    PianoAppTheme(darkTheme = SettingsObject.darkTheme) {
        Surface(
            modifier = Modifier.fillMaxSize(),color = MaterialTheme.colorScheme.background
        ) {
            Column {
                TopBar(title = ScreensEnum.FUNFACT.namePl)
                Card(modifier = AppAppearance.cardModifier) {
                    Column(horizontalAlignment = Alignment.CenterHorizontally,
                        ...
                        Row(modifier = Modifier.fillMaxWidth(),
                            ...
                            IconButton(onClick = {
                                favouriteBoolean = favouriteBoolean.not()
                                val newFunFact = FunFact(
                                    id = id,
                                    title = title,
                                    text = text,
                                    source = source,
                ...
                IconButton(onClick = { Navigation.goTo(ScreensEnum.FUNFACTS) }) {
                    Icon(
